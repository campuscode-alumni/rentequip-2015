...
feature 'User create equipment category' do
  ...
  scenario 'click_on Voltar' do
    ...
    click_on 'Voltar'
    expect(page).to have_content "##{equipment_category1.id} - #{equipment_category1.name}"
    expect(page).to have_content "##{equipment_category2.id} - #{equipment_category2.name}"
  end
